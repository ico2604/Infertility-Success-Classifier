...
# -*- coding: utf-8 -*-
"""
v19 - v17 기반 + 배아이식일_있음 플래그 + 노이즈 피처 7개 제거
==========================================================================
v17 대비 변경 (2가지만):
  1. 배아이식일_있음 결측 플래그 추가 (v18에서 중요도 #8 확인)
  2. 노이즈 피처 7개 제거 (v17에서 중요도 0.00 확인)
모든 HP, fillna(0), border_count 64 등은 v17 그대로 유지.
"""
...
RESULT_DIR = os.path.join(BASE_DIR, "result_v19")
...
# ============================================================
log(f"# {VERSION} - v17 + 배아이식일_있음 + 노이즈제거")
log(f"시각: {NOW}")
log("=" * 60)
...
def preprocess_full(df, is_train=True):
    ...
    # [v19 수정1] 배아이식일_있음 플래그 — fillna(0) 전에 원본 NaN 기준으로 생성
    if "배아 이식 경과일" in df.columns:
        df["배아이식일_있음"] = (~df["배아 이식 경과일"].isna()).astype(int)

    # 수치형 NaN → 0 (v17 그대로 유지 — HP 호환성)
    ...
    # [v19 수정2] 노이즈 피처 7개 제거 (v17에서 중요도 0.00 확인)
    drop_noise = [
...
# v19 검증
log(f"\n### v19 변경사항 검증:")
log(f"  - border_count: {CB_PARAMS['border_count']} (v17 동일)")
if "배아이식일_있음" in X_train.columns:
    log(f"  - 배아이식일_있음: 1={X_train['배아이식일_있음'].sum()}, 0={(X_train['배아이식일_있음']==0).sum()}")
else:
    log(f"  - 배아이식일_있음: NOT FOUND (오류!)")

# v17 frozen 피처 (수정2에서 2개 제거됨)
v17_new = [
    "is_frozen_transfer",
...
# v19 신규 피처
v19_new = ["배아이식일_있음"]
v19_found = [f for f in v19_new if f in X_train.columns]
log(f"- v19 신규 피처 ({len(v19_found)}개): {v19_found}")

# 팀원 피처 확인
team_feats = [
    "배아생성효율", "ICSI수정효율", "배아이식비율", "배아저장비율", "난자활용률",
    "난자대비이식배아수", "이식배아수_구간", "전체임신률", "IVF임신률", "DI임신률",
    "임신유지율", "IVF임신유지율", "총실패횟수", "IVF실패횟수", "반복IVF실패_여부",
    "클리닉집중도", "IVF시술비율", "임신경험있음", "출산경험있음", "나이_제곱",
    "나이_임상구간", "고령_여부", "초고령_여부", "극고령_여부", "나이X총시술",
    "나이XIVF실패", "나이XIVF임신률", "초고령X반복실패", "복합위험도점수",
]
...
if age_col in train_raw.columns:
    for av in ["만18-34세", "만35-37세", "만38-39세", "만40-42세", "만43-44세", "만45-50세"]:
...
for th in [0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50]:
    ...
    log(f"  {th:>6.2f}  {acc:>7.4f}  {prec:>7.4f}  {rec:>7.4f}  {f1:>7.4f}  {spec:>7.4f}")
    if f1 > best_f1:
...
log(f"- 파일: {sub_path}")
log(f"- 확률: mean={test_cb.mean():.4f}, std={test_cb.std():.4f}, min={test_cb.min():.6f}, max={test_cb.max():.4f}")
...
if cb_importances is not None:
    ...
    for rank_i, (_, row) in enumerate(imp_df.head(30).iterrows(), 1):
        ...
        if row["feature"] in v19_found:
            marks += " ★v19"
        elif row["feature"] in v17_found:
            marks += " ★v17"
    ...
    log(f"\n### 중요도 0 피처 ({len(zero_imp)}개): {zero_imp[:20]}")

    imp_df.to_csv(os.path.join(RESULT_DIR, f"feature_importance_{VERSION}.csv"), index=False)
...
gate1 = cb_auc > 0.740816
log(f"- Gate 1: OOF AUC {cb_auc:.6f} > 0.740816 → {'PASS' if gate1 else 'FAIL'}")
...
if frozen_transfer_train.sum() > 50:
    ...
    log(f"- Gate 2: frozen AUC {frozen_auc:.4f} >= 0.6201 → {'PASS' if gate2 else 'FAIL'}")
else:
    gate2 = False

log(f"- 피처 수: {len(X_train.columns)} (목표 139)")
...
log(f"{'='*60}")
log(f"- v19 변경사항 (2가지만):")
log(f"  1. 배아이식일_있음 플래그 추가")
log(f"  2. 노이즈 피처 7개 제거")
log(f"- CB 5-seed OOF AUC: {cb_auc:.6f}")
log(f"  seed별: {[f'{a:.4f}' for a in seed_aucs]}")
log(f"- v17 대비: {cb_auc - 0.740816:+.6f}")
