# =========================================================
# XGBoost Stratified K-Fold Validation v4 — Feature Lite
# ---------------------------------------------------------
# 목적
# 1) train/test 데이터를 불러온다.
# 2) 기본 수치 변환(횟수형/나이형)을 수행한다.
# 3) IVF 의료 도메인 기반 파생변수를 생성한다.
# 4) v3에서 importance가 높았던 소수의 추가 feature만 생성한다.
...
#
# 실행 방법
#   python yysop/src/xgb_kfold_v4.py
# =========================================================
...
# 1. 실험 설정
# =========================================================
EXPERIMENT_NAME = "reg_relax_feat_lite"
...
def add_interaction_features_v4_lite(df):
    def has(*cols):
        return all(c in df.columns for c in cols)
...
train = add_interaction_features_v4_lite(train)
test = add_interaction_features_v4_lite(test)
...
log_path = os.path.join(OUTPUT_DIR, "xgb_v4_results_log.csv")
...
imp_path = os.path.join(OUTPUT_DIR, f"xgb_v4_{EXPERIMENT_NAME}_feature_importance.csv")
...
roc_path = os.path.join(OUTPUT_DIR, f"xgb_v4_{EXPERIMENT_NAME}_roc_curve.png")
...
pr_path = os.path.join(OUTPUT_DIR, f"xgb_v4_{EXPERIMENT_NAME}_pr_curve.png")
...
oof_path = os.path.join(OUTPUT_DIR, f"xgb_v4_{EXPERIMENT_NAME}_oof_predictions.csv")
...
threshold_path = os.path.join(OUTPUT_DIR, f"xgb_v4_{EXPERIMENT_NAME}_threshold_analysis.csv")
...
sub_path = os.path.join(OUTPUT_DIR, f"xgb_v4_{EXPERIMENT_NAME}_submission.csv")
...
print("\n저장 파일:")
print(f"  1. {log_path}          ← v4 실험 누적 비교표")
print(f"  2. {imp_path}")
print(f"  3. {roc_path}")
print(f"  4. {pr_path}")
print(f"  5. {oof_path}")
...
print("\n다음 추천 실험:")
print("  - CatBoost도 동일 feature lite 버전(v4) 생성")
print("  - XGB v4 결과와 CAT v4 결과로 앙상블 재확인")
print("  - Optuna는 v2 reg_relax 또는 v4 lite 기준으로 진행")