...
# ---------------------------------------------------------
# 목적
# 1) xgb_kfold_v2.py의 전처리/파생변수 흐름을 그대로 사용한다.
# 2) Optuna로 XGBoost 하이퍼파라미터를 탐색한다.
# 3) Stratified K-Fold OOF AUC 기준으로 best params를 찾는다.
# 4) best params로 전체 CV 재학습 후 OOF / submission / importance 저장
# =========================================================
...
def objective(trial):
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf_optuna.split(X, y), start=1):
...
print(f"Best OOF AUC      : {study.best_value:.6f}")
print("Best params:")
...
# =========================================================
# 10. Best params로 최종 CV 재학습
# =========================================================
print_section("10. Best params 최종 CV 재학습")

best_params = study.best_params.copy()
best_params["early_stopping_rounds"] = FINAL_EARLY_STOPPING_ROUNDS
...
# 12. 결과 저장
# =========================================================
print_section("12. 결과 저장")

best_params_path = os.path.join(OUTPUT_DIR, f"{EXPERIMENT_NAME}_best_params.json")
with open(best_params_path, "w", encoding="utf-8") as f:
    json.dump({
        "best_trial_number": study.best_trial.number,
        "best_optuna_value": study.best_value,
        "best_params": study.best_params,
        "final_train_params": FINAL_XGB_PARAMS,
    }, f, ensure_ascii=False, indent=2)
print(f"[저장] {best_params_path}")

log_path = os.path.join(OUTPUT_DIR, "xgb_optuna_v1_results_log.csv")
log_row = {
