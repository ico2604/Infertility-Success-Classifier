# =========================================================
# XGBoost Stratified K-Fold Validation v3 — Feature Engineering 확장
# ---------------------------------------------------------
# 목적
# 1) train/test 데이터를 불러온다.
# 2) 기본 수치 변환(횟수형/나이형)을 수행한다.
# 3) IVF 의료 도메인 기반 파생변수를 생성한다.
# 4) 추가 interaction / ratio / binning 파생변수를 생성한다.
# 5) 범주형 컬럼을 안전하게 숫자형으로 인코딩한다.
# 6) Stratified K-Fold + OOF 방식으로 안정적인 내부 검증을 수행한다.
# 7) 실험 결과를 누적 로그로 저장한다.
# 8) Feature Importance / ROC Curve / OOF / Submission 파일을 저장한다.
#
# 실행 방법
#   python yysop/src/xgb_kfold_v3.py
# =========================================================
...
# 1. 실험 설정
# =========================================================
# v3에서는 v2의 좋은 설정(reg_relax)을 기반으로 feature engineering 확장
EXPERIMENT_NAME = "reg_relax_feat_plus"
...
BASE_EXPERIMENT_KEY = "reg_relax"
assert BASE_EXPERIMENT_KEY in EXPERIMENTS, f"BASE_EXPERIMENT_KEY '{BASE_EXPERIMENT_KEY}'이 EXPERIMENTS에 없습니다."
TUNE_PARAMS = EXPERIMENTS[BASE_EXPERIMENT_KEY]

print(f"\n실험 이름: {EXPERIMENT_NAME}")
print(f"기반 실험: {BASE_EXPERIMENT_KEY}")
print("파라미터:")
...
def add_interaction_features_v3(df):
    def has(*cols):
        return all(c in df.columns for c in cols)
    ...
    # [F] 난자 → 배아 전환 품질
    # - 중복이던 난자대비배아수_v3 제거
    # - 기존 v2와 계산 철학 맞추기 위해 +1 smoothing 제거
    # -----------------------------------------------------
    if has("총 생성 배아 수", "수집된 신선 난자 수"):
        df["배아생성률_v3"] = safe_div(
            df["총 생성 배아 수"],
            df["수집된 신선 난자 수"]
        )

    if has("미세주입에서 생성된 배아 수", "미세주입된 난자 수"):
        df["ICSI효율_v3"] = safe_div(
            df["미세주입에서 생성된 배아 수"],
            df["미세주입된 난자 수"]
        )
    ...
    # -----------------------------------------------------
    if has("이식된 배아 수", "총 생성 배아 수"):
        df["배아이식효율_v3"] = safe_div(
            df["이식된 배아 수"],
            df["총 생성 배아 수"]
        )

    if has("이식된 배아 수", "저장된 배아 수", "총 생성 배아 수"):
        df["배아활용률_v3"] = safe_div(
    ...
    # [H] 나이 interaction
    # -----------------------------------------------------
    if has("이식된 배아 수", "시술 당시 나이_num"):
        df["이식강도_v3"] = (
            df["이식된 배아 수"].fillna(0) *
            df["시술 당시 나이_num"].fillna(0)
        )

    if has("시술 당시 나이_num", "총 생성 배아 수"):
        df["나이X배아생성_v3"] = (
            df["시술 당시 나이_num"].fillna(0) *
            df["총 생성 배아 수"].fillna(0)
        )

    if has("시술 당시 나이_num", "수집된 신선 난자 수"):
        df["나이X난자_v3"] = (
            df["시술 당시 나이_num"].fillna(0) *
    ...
    # [I] 실패 / 시술 강도
    # -----------------------------------------------------
    if has("IVF실패횟수", "IVF 시술 횟수_num"):
        df["IVF실패비율_v3"] = safe_div(
            df["IVF실패횟수"],
            df["IVF 시술 횟수_num"]
        )

    if has("총 시술 횟수_num", "시술 당시 나이_num"):
        df["시술강도_v3"] = (
    ...
    # 복합위험도점수는 0일 수 있으므로 여기만 +1 유지
    if has("복합위험도점수", "총 생성 배아 수"):
        df["복합위험도대비배아_v3"] = safe_div(
            df["총 생성 배아 수"],
            df["복합위험도점수"] + 1
        )
...
train = add_interaction_features_v3(train)
test = add_interaction_features_v3(test)
...
log_path = os.path.join(OUTPUT_DIR, "xgb_v3_results_log.csv")
...
print("\n[현재까지 실험 요약]")
print(log_df[[
    "timestamp", "experiment", "base_experiment_key",
    "oof_auc", "mean_auc", "std_auc", "n_features", "n_new_features"
...
imp_path = os.path.join(OUTPUT_DIR, f"xgb_v3_{EXPERIMENT_NAME}_feature_importance.csv")
...
roc_path = os.path.join(OUTPUT_DIR, f"xgb_v3_{EXPERIMENT_NAME}_roc_curve.png")
...
pr_path = os.path.join(OUTPUT_DIR, f"xgb_v3_{EXPERIMENT_NAME}_pr_curve.png")
...
oof_path = os.path.join(OUTPUT_DIR, f"xgb_v3_{EXPERIMENT_NAME}_oof_predictions.csv")
...
threshold_path = os.path.join(OUTPUT_DIR, f"xgb_v3_{EXPERIMENT_NAME}_threshold_analysis.csv")
...
sub_path = os.path.join(OUTPUT_DIR, f"xgb_v3_{EXPERIMENT_NAME}_submission.csv")
...
print(f"base key: {BASE_EXPERIMENT_KEY}")
...
print("\n저장 파일:")
print(f"  1. {log_path}          ← v3 실험 누적 비교표")
print(f"  2. {imp_path}")
print(f"  3. {roc_path}")
print(f"  4. {pr_path}")
print(f"  5. {oof_path}")
...
print("\n다음 추천 실험:")
print("  - CatBoost도 동일 feature 확장 버전(v3) 생성")
print("  - XGB v3 결과와 CAT v3 결과로 앙상블 재확인")
print("  - 새로 추가된 feature importance 상위권 진입 여부 확인")