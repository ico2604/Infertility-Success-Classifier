# =========================================================
# CatBoost Stratified K-Fold Validation + Optuna v4 Light
# ---------------------------------------------------------
# 목적
...
# 5) Optuna는 3-Fold로 가볍게 탐색하여 좋은 파라미터를 빠르게 찾는다.
# 6) Best Params로 5-Fold 최종 학습을 수행한다.
# 7) 실험 결과를 누적 로그로 저장한다.
# 8) Feature Importance / OOF / Submission 파일을 저장한다.
#
# 핵심 포인트
# - 기존 catboost_v3 pipeline 최대한 유지
# - COUNT/AGE 원본 문자열 컬럼 제거
# - 일반 범주형 문자열은 CatBoost에 그대로 전달
# - OOF 파일과 제출 파일의 target 컬럼명을 분리 관리
# - Optuna 탐색은 3-Fold / 최종 검증은 5-Fold
# - iterations는 탐색 시 고정하고 early stopping으로 제어
# =========================================================
...
EXPERIMENT_NAME = "catboost_optuna_v4_light"
...
# 탐색용 고정 설정
OPTUNA_FIXED_ITERATIONS = 3000
OPTUNA_EARLY_STOPPING_ROUNDS = 100

# 최종 학습 시 verbose / early stopping
FINAL_VERBOSE = 200
...
# Optuna용 3-Fold CV split 고정
cv_splits = list(
    StratifiedKFold(
        n_splits=OPTUNA_N_FOLDS,
        shuffle=True,
        random_state=SEED
    ).split(X, y)
)
...
def build_cat_params(trial):
    params = {
        "iterations": OPTUNA_FIXED_ITERATIONS,
...
print(f"OPTUNA_N_FOLDS        : {OPTUNA_N_FOLDS}")
print(f"FINAL_N_FOLDS         : {FINAL_N_FOLDS}")
print(f"OPTUNA_FIXED_ITERATIONS: {OPTUNA_FIXED_ITERATIONS}")
    ...
    json.dump(
        {
            ...
            "optuna_n_folds": OPTUNA_N_FOLDS,
            "final_n_folds": FINAL_N_FOLDS,
            "optuna_fixed_iterations": OPTUNA_FIXED_ITERATIONS,
        },
        f,
        ensure_ascii=False,
        indent=2
...
FINAL_CAT_PARAMS["early_stopping_rounds"] = FINAL_EARLY_STOPPING_ROUNDS
...
log_path = os.path.join(OUTPUT_DIR, "catboost_optuna_v4_light_results_log.csv")
...
print(f"Optuna Fold 수       : {OPTUNA_N_FOLDS}")
print(f"Final Fold 수        : {FINAL_N_FOLDS}")
...
print("\n고정 설정:")
print(f"  - OPTUNA_FIXED_ITERATIONS     : {OPTUNA_FIXED_ITERATIONS}")
print(f"  - OPTUNA_EARLY_STOPPING_ROUNDS: {OPTUNA_EARLY_STOPPING_ROUNDS}")
print(f"  - FINAL_EARLY_STOPPING_ROUNDS : {FINAL_EARLY_STOPPING_ROUNDS}")
