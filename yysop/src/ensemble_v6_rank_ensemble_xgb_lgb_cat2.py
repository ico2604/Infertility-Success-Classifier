...
# 3) 3모델 rank ensemble(equal / weighted)을 평가한다.
...
RESULT_LOG_PATH = os.path.join(OUTPUT_DIR, "ensemble_v6_rank_results_log.csv")
SEARCH_RESULT_PATH = os.path.join(OUTPUT_DIR, "ensemble_v6_rank_search_results.csv")
...
EXPERIMENT_NAME = "ensemble_v6_rank_ensemble_xgb_lgb_cat2"
...
def generate_weight_candidates_3(step=0.02):
    ...
    for wx in grid:
        for wl in grid:
            ...
            w3_r = round(float(w3), ROUND_DIGITS)

            if abs((wx_r + wl_r + w3_r) - 1.0) > 1e-8:
                continue

            weights.append((wx_r, wl_r, w3_r))
...
for i, (wx, wl, w3) in enumerate(weight_candidates, start=1):
    pred = wx * xgb_rank + wl * lgb_rank + w3 * cat2_rank
...
rank_oof_path = os.path.join(
    OUTPUT_DIR,
    f"ensemble_v6_rank_best_xgb{int(round(wx * 100))}_"
    f"lgb{int(round(wl * 100))}_cat2{int(round(w3 * 100))}_oof.csv"
)
rank_sub_path = os.path.join(
    OUTPUT_DIR,
    f"ensemble_v6_rank_best_xgb{int(round(wx * 100))}_"
    f"lgb{int(round(wl * 100))}_cat2{int(round(w3 * 100))}_submission.csv"
)
...
corr_path = os.path.join(OUTPUT_DIR, "ensemble_v6_oof_correlation.csv")
...
if len(imp_frames) >= 2:
    ...
    ensemble_imp_path = os.path.join(OUTPUT_DIR, "ensemble_v6_feature_importance.csv")
...
log_row = {
    ...
    "best_cat2_weight": round(float(best_weights[2]), 4),
