...
RESULT_LOG_PATH = os.path.join(OUTPUT_DIR, "ensemble_v4_rank_xgb_lgbivf_catv6_results_log.csv")
SEARCH_RESULT_PATH = os.path.join(OUTPUT_DIR, "ensemble_v4_rank_xgb_lgbivf_catv6_search_results.csv")
...
# baseline 비교용 설정
# ---------------------------------------------------------
# False면 현재 조합(XGB+CATv6+LGB_ivf)의 probability weighted baseline을
# 간단 탐색해서 비교함
...
FIXED_BASELINE_NAME = "rank_xgb_lgb_catv6_prev"
FIXED_BASELINE_WEIGHTS = "xgb=0.34, cat=0.52, lgb=0.14"

# rank ensemble 가중치 탐색 step
SEARCH_STEP = 0.01
ROUND_DIGITS = 4

EXPERIMENT_NAME = "ensemble_v4_rank_xgb_lgbivf_catv6"
...
rank_oof_path = os.path.join(
    OUTPUT_DIR,
    f"ensemble_v4_rank_xgb_lgbivf_catv6_best_xgb{int(round(wx * 100))}_cat{int(round(wc * 100))}_lgb{int(round(wl * 100))}_oof.csv"
)
rank_sub_path = os.path.join(
    OUTPUT_DIR,
    f"ensemble_v4_rank_xgb_lgbivf_catv6_best_xgb{int(round(wx * 100))}_cat{int(round(wc * 100))}_lgb{int(round(wl * 100))}_submission.csv"
)
...
# cat AUC                 : 0.740342
# lgb AUC                 : 0.739883
# prob equal ensemble AUC : 0.740573
# prob_weighted_current_3model AUC     : 0.740631
# baseline weights        : xgb=0.33, cat=0.52, lgb=0.15

# 7. 결과 요약 / baseline 비교
# ====================================================================================================
# best single auc          : 0.740342
# prob equal auc           : 0.740573
# rank equal auc           : 0.740576
# prob weighted baseline   : 0.740631
# best weighted rank auc   : 0.740634

# gain vs prob baseline    : +0.000003
# gain vs rank equal       : +0.000058
# gain vs prob equal       : +0.000061
# gain vs best single      : +0.000292
