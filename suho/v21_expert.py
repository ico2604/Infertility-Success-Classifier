...
EXTERNAL_OOF_TEMPLATE = os.path.join(RESULT_DIR, "oof_{ver}_final.npy")
EXTERNAL_TEST_TEMPLATE = os.path.join(RESULT_DIR, "test_{ver}_final.npy")
...
def load_external_global_blend():
    ...
    for ver, w in EXTERNAL_GLOBAL_BLEND.items():
        oof_path = EXTERNAL_OOF_TEMPLATE.format(ver=ver)
        test_path = EXTERNAL_TEST_TEMPLATE.format(ver=ver)
...
def search_expert_blend(
    ...
    """
    pred = global
    - non-frozen transfer: (1-a) * global + a * transfer
    - frozen transfer: (1-b-c) * global + b * transfer + c * frozen
      단, b+c <= 1
    """
    ...
    for a in grid_a:
        base_pred = global_oof.copy()
        base_pred[non_frozen_transfer] = (1 - a) * global_oof[
            non_frozen_transfer
...
def log_importance_block(imp_arr, tag, topn=30):
    if imp_arr is None or len(imp_arr) != len(feature_names):
        return None

    imp_df = pd.DataFrame(
        {"feature": feature_names, "importance": imp_arr}
...
transfer_imp_df = log_importance_block(transfer_imp, "transfer_expert", topn=35)
frozen_imp_df = log_importance_block(frozen_imp, "frozen_expert", topn=35)
...
log(
    f"| **{VERSION}** | **expert blend** | **{final_auc:.6f}** | **TBD** | **global+transfer+frozen** |"
)
...
log("  3. transfer-only expert 모델 추가")
log("  4. frozen-transfer-only expert 모델 추가")
log("  5. OOF 기반 gating/blending weight 자동 탐색")
log(f"  6. best weights: {best_weights}")
