...
# -*- coding: utf-8 -*-
"""
v17 - 팀원 XGB 피처 통합 + frozen transfer 피처 + class_weights + CB 5-seed
==========================================================================
핵심 변경:
  1. 팀원 XGB 코드의 피처 엔지니어링 완전 통합 (make_medical_features)
  2. frozen transfer 전용 피처 추가 (아이디어 1)
  3. CatBoost class_weights {0:1.0, 1:1.3} 적용 (아이디어 4)
  4. 카테고리 20개 복원 (횟수형 원본 유지)
  5. 중요도 0 피처 제거 (140→정리)
  6. CatBoost 5-seed 앙상블
"""
...
# ============================================================
log(f"# {VERSION} - 팀원 XGB 피처 + frozen 피처 + class_weights + CB 5-seed")
log(f"시각: {NOW}")
log("=" * 60)
...
def preprocess_full(df, is_train=True):
    ...
    if type_col in df.columns:
        ...
        np.place(
            df["ivf_transfer_ratio"].values,
            ivf_mask,
            safe_div_arr(embryo[ivf_mask], total_emb[ivf_mask]),
        )
        df["ivf_storage_ratio"] = 0.0
        np.place(
            df["ivf_storage_ratio"].values,
            ivf_mask,
            safe_div_arr(stored[ivf_mask], total_emb[ivf_mask]),
        )
...
log(f"- v17 frozen 신규 피처 ({len(v17_found)}개): {v17_found}")
...
log(
    f"| **{VERSION}** | **CB 5seed** | **{cb_auc:.4f}** | **TBD** | **팀원피처+frozen+CW** |"
)
...
log(f"{'='*60}")
log(f"- v17 핵심 변경:")
log(f"  1. 팀원 XGB 의료 파생변수 {len(team_found)}개 통합")
log(f"  2. frozen transfer 전용 피처 {len(v17_found)}개 추가")
log(f"  3. class_weights {{0:1.0, 1:1.3}} 적용")
log(f"  4. 카테고리 20개 복원 (횟수형 원본 유지)")
log(f"  5. CatBoost 5-seed 앙상블 (seed별 다른 fold)")
